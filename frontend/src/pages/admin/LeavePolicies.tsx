...
export const LeavePolicies = () => {
    ...
    const [formData, setFormData] = useState({ leaveType: 'CASUAL', maxDaysPerYear: 0, carryForwardLimit: 0 });
    ...
    const fetchPolicies = () => {
        client.get('/leave-policies').then(res => setPolicies(res.data.data)).catch(console.error);
    };

    const handleDelete = async (id: string) => {
        if (!confirm('Are you sure?')) return;
        try {
            await client.delete(`/leave-policies/${id}`);
            fetchPolicies();
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await client.post('/leave-policies', {
                ...formData,
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'white' }}>Leave Policies</h2>
                <button
            ...
            <div style={{
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                        <tr style={{ background: 'rgba(0, 0, 0, 0.2)', textAlign: 'left' }}>
                            ...
                            <th style={{ padding: '1rem', color: '#D1D5DB' }}>Max Days / Year</th>
                            <th style={{ padding: '1rem', color: '#D1D5DB' }}>Carry Forward Limit</th>
                    ...
                    <tbody>
                        {policies.map(policy => (
                            <tr key={policy.id} style={{ borderBottom: '1px solid rgba(255, 255, 255, 0.05)' }}>
                                <td style={{ padding: '1rem', fontWeight: 600, color: 'white' }}>{policy.leaveType}</td>
                                <td style={{ padding: '1rem', color: '#E5E7EB' }}>{policy.maxDaysPerYear}</td>
                                <td style={{ padding: '1rem', color: '#E5E7EB' }}>{policy.carryForwardLimit}</td>
                                <td style={{ padding: '1rem', display: 'flex', gap: '1rem' }}>
                                    <button onClick={() => handleDelete(policy.id)} style={{ color: '#F87171', background: 'none', border: 'none', cursor: 'pointer' }}>
                                        <Trash2 size={18} />
                                    </button>
                                </td>
                            </tr>
                ...
                {policies.length === 0 && <p style={{ textAlign: 'center', padding: '2rem', color: '#9CA3AF' }}>No policies configured yet.</p>}
            </div>
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(5px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
                    ...
                    >
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
                            <h3 style={{ margin: 0, fontSize: '1.25rem' }}>Configure Policy</h3>
                        ...
                        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <div>
                                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: 600, color: '#D1D5DB' }}>Leave Type</label>
                                <select
                                ...
                                >
                                    ...
                                    <option value="EARNED" style={{ color: 'black' }}>Earned Leave</option>
                                    <option value="UNPAID" style={{ color: 'black' }}>Unpaid Leave</option>
                                </select>
                            </div>
                            <div>
                                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: 600, color: '#D1D5DB' }}>Max Days / Year</label>
                                <input
                                    type="number"
                            ...
                            <div>
                                <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.9rem', fontWeight: 600, color: '#D1D5DB' }}>Carry Forward Limit</label>
                                <input
                                    type="number"
                                    value={formData.carryForwardLimit}
                                    onChange={e => setFormData({ ...formData, carryForwardLimit: Number(e.target.value) })}
                                    style={{ width: '100%', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid rgba(255, 255, 255, 0.1)', background: 'rgba(0,0,0,0.2)', color: 'white' }}
                                />
                            </div>
